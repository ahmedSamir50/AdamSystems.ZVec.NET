...
Samples use SDK **`factory.OpenOrCreate`** (upstream `CreateAndOpen` throws if the path exists — same as Python/Node). Second launch opens existing collections under AppData / temp. The SDK loads managed schema from on-disk metadata so Query/Fetch return Title/ChunkText/etc., not scores alone. If an old collection still looks empty, delete the three `zvec-samples-*` folders once and re-seed.
